...
CSceneTestSaver::CSceneTestSaver(CDisplayContext *poDisplay) : 
	CScene(poDisplay),
	m_poAvatar(NULL),
	m_bGeneric(true),
	m_poRAvatar(NULL),
...
CSceneTestSaver::~CSceneTestSaver() {
	if (m_poRC)
		Destroy();
} // Destructor

SCRESULT CSceneTestSaver::Create() {
	...
	NEWEND("CSceneTestSaver::Create - Avatar render object")
	// Create the light object
	NEWBEGIN
	m_poRLight = (CRenderLight*) new CRenderLight(m_poRC);
	NEWEND("CSceneTestSaver::Create - Light render object")
	// Create the camera object
	NEWBEGIN
	m_poRCamera = (CRenderCamera*) new CRenderCamera(m_poRC);
	NEWEND("CSceneTestSaver::Create - Camera render object")
	// Check we're ok
...
SCRESULT CSceneTestSaver::Destroy() {
...
SCRESULT CSceneTestSaver::ImportAvatar(CAvatar *poAvatar, bool bGeneric) {
...
SCRESULT CSceneTestSaver::ImportPose(CAvatarPose &oPose) {
	// Create a copy of the pose
	CAvatarPose *poPose = NULL;
	NEWBEGIN
	poPose = (CAvatarPose*) new CAvatarPose(oPose);
	NEWEND("CSceneTestSaver::ImportPose - New pose object");
	if (!poPose)
		return SC_OUT_OF_MEMORY;
...
SCRESULT CSceneTestSaver::Render() {
...
SCRESULT CSceneTestSaver::Snapshot(CImage *&poImage) {
	// Check for a render context
	if (!m_poRC)
		return SC_NO_CONTEXT;
	// Get the snapshot
	...
	if (eResult != RC_OK) {
		TRACE("CSceneTestSaver::Snapshot - %s\n", m_poRC->GetErrorString(eResult));
		return SC_SNAPSHOT_ERROR;
	}
	return SC_OK;
} // Snapshot

SCRESULT CSceneTestSaver::SetSize(int iWidth, int iHeight) {
	// Check for a render context
	if (!m_poRC)
		return SC_NO_CONTEXT;
	// Set the size
