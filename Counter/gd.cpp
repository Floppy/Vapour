...
//                fputc( Blue[i], fp );
					  fwrite(&Red[i], 1, 1, fp);
					  fwrite(&Green[i], 1, 1, fp);
					  fwrite(&Blue[i], 1, 1, fp);
        }
...
//	    fputc( 0, fp );
		const char pcData[6] = { '!', 0xF9, 4, 1, 0, 0 };
		fwrite(pcData, 1, 6, fp);
		fwrite(&Transparent, 1, 1, fp);
		fwrite(&cZero, 1, 1, fp);
	}
...
//        else
//                fputc( 0x00, fp );
		  const char cInterlace = 0x40;
        if( Interlace )
			  fwrite(&cInterlace, 1, 1, fp);
        else
			  fwrite(&cZero, 1, 1, fp);
...
//        fputc( w & 0xff, fp );
//        fputc( (w / 256) & 0xff, fp );
	const char pcWord[2] = { w & 0xff, (w / 256) & 0xff };
	fwrite(pcWord, 1, 2, fp);
}
